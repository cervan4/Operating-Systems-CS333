...
allocproc(void)
{
  ...
  if(stateListRemove(&ptable.list[UNUSED],p) == -1 ){
    panic("failed to remove from UNUSED list after kernal stack allocation failure in allocationproc()");
  }
  ...
  if(stateListRemove(&ptable.list[EMBRYO],p) == -1){
    panic("failed to remove from EMBRYO list after kernal stack allocation failure in allocationproc()");
  }
  acquire(&ptable.lock);
...
#ifdef CS333_P3
  if(stateListRemove(&ptable.list[EMBRYO],p) == -1) {
     panic("failed to remove from EMBRYO list after successful allcocation in userinit()");
  }
  assertState(p,EMBRYO, __FUNCTION__, __LINE__);
#endif
  p->state = RUNNABLE;
...
fork(void)
{
  ...
  if((np->pgdir = copyuvm(curproc->pgdir, curproc->sz)) == 0){
    ...
    if(stateListRemove(&ptable.list[EMBRYO],np) ==-1) {
      panic("failed to remove from EMBRYO list fork() after page directory allocation failure");
    }
...
getprocs(uint max, struct uproc * table)
{
 ...
 struct proc * my_proc = myproc();
 acquire(&ptable.lock); 
 for(my_proc = ptable.proc; my_proc < &ptable.proc[NPROC] && loop < max; my_proc++)
 {
  if(my_proc->state == SLEEPING || my_proc->state == RUNNING || my_proc->state == RUNNABLE ||my_proc->state == ZOMBIE)
  {
   table[loop].pid = my_proc->pid; //pid
   table[loop].uid = my_proc->uid; //uid
   table[loop].gid = my_proc->gid; //gid
   if(my_proc ->pid == 1)
   {
    table[loop].ppid = 1;
   }
   ...
   {
     table[loop].ppid = my_proc->parent->pid; //ppid
   }
   table[loop].elapsed_ticks = ticks - my_proc->start_ticks; //elapsed ticks
   table[loop].CPU_total_ticks = my_proc->cpu_ticks_total; // CPU time
   table[loop].size = my_proc->sz; // size
   safestrcpy(table[loop].name,my_proc->name,STRMAX); // name
   safestrcpy(table[loop].state,states[my_proc->state],STRMAX); //state 
   ++loop;
  }
 }
 release(&ptable.lock);
...
/*
int
display(uint max,struct uproc * table)
{
 int loop = 0;
 struct proc * my_proc = myproc();
 acquire(&ptable.lock); 
 for(my_proc = ptable.proc; my_proc < &ptable.proc[NPROC] && loop < max; my_proc++)
 {
  ...
  {
   table[loop].pid = my_proc->pid; //pid
   printf("pid: %d",table[loop].pid);
   table[loop].uid = my_proc->uid; //uid
   table[loop].gid = my_proc->gid; //gid
   table[loop].ppid = my_proc->parent->pid; //ppid
   table[loop].elapsed_ticks = my_proc->elapsed_ticks; //elapsed ticks
   table[loop].CPU_total_ticks = my_proc->cpu_ticks_total; // CPU time
   table[loop].size = my_proc->sz; // size
   safestrcpy(table[loop].name,my_proc->name,STRMAX); // name
   safestrcpy(table[loop].state,states[my_proc->state],STRMAX); //state 
...
exit(void) {
  ...
  if(stateListRemove(&ptable.list[RUNNING],curproc) == -1) {
     panic("failed to remove from RUNNING list after successful allcocation in exit()");
  }
...
//
        if(stateListRemove(&ptable.list[ZOMBIE],p) == -1) {
          panic("failed to remove from ZOMBIE list after successful allcocation in wait()");
        } 
  ...
  for(;;){
    ...
    if(p) { 
      ...
      if(stateListRemove(&ptable.list[RUNNABLE], p) == -1) {
        panic("failed to remove process we will run form RUNNABLE list in scheduler()");
      }
...
#ifdef CS333_P3
  if(stateListRemove(&ptable.list[RUNNING],curproc) == -1) {
     panic("failed to remove from EMBRYO list after successful allcocation in yield()");
  }
  assertState(curproc,RUNNING, __FUNCTION__, __LINE__);
#endif
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  if(stateListRemove(&ptable.list[RUNNING],p) == -1) {
     panic("failed to remove from RUNNING list after successful allcocation in sleep()");
  }
...
wakeup1(void *chan)
{
  ...
  if(stateListRemove(&ptable.list[SLEEPING],p) == -1) {
     panic("failed to remove from RUNNING list after successful allcocation in wakeup1()");
  }
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if(p->pid == pid){
      ...
      if(p->state == SLEEPING) {
        ...
        // that we need to wake up? if we remove p from the sleeping list what does p->next become?
  if(stateListRemove(&ptable.list[SLEEPING],p) == -1) {
     panic("failed to remove from RUNNING list after successful allcocation in kill()");
  }
  assertState(p,SLEEPING, __FUNCTION__, __LINE__);
        p->state = RUNNABLE;
        stateListAdd(&ptable.list[RUNNABLE], p);
...
procdumpP2P3P4(struct proc *p, char *state_string)
{
   ...
   cprintf("%d.%d\t",p->cpu_ticks_in);
   cprintf("%d.%d\t",p->cpu_ticks_total);
...
procdumpP1(struct proc *p, char *state_string)
{
  ...
  //int elapse = ticks % 10;
  double remain = (ticks-p->start_ticks)/1000;
  cprintf("%d",p->pid);
  cprintf("      %s",p->name);
  cprintf("          %d.%d",remain);
  cprintf("          %s",state_string);
